...
# Define multiple group names (user can modify this list)
...
for group in group_names:
    ...
    # Extract forces and section names for selected frames
    data = []
    for frame in group_frames:
        _, section_name = SapModel.FrameObj.GetSection(frame)  # Get section name
        _, _, _, P, V2, V3, T, M2, M3 = SapModel.Results.FrameForce(frame, 0)
        data.append([frame, section_name, P, V2, V3, T, M2, M3])

    # Create Pandas DataFrame for the group
    columns = ["Frame", "Section Name", "Axial Force (P)", "Shear Force V2", "Shear Force V3", "Torsion (T)", "Moment M2", "Moment M3"]
